...
export const useGestureEventsHandlersDefault = () => {
  ...
  const context = useSharedValue({ ...INITIAL_CONTEXT
  }); //#endregion
  //#region gesture methods

  const handleOnStart = useWorkletCallback(function handleOnStart(__, _ref) {
    let {
      translationY
    } = _ref;
  ...
  const handleOnChange = useWorkletCallback(function handleOnChange(source, _ref2) {
    let {
      translationY
    } = _ref2;
  ...
  const handleOnEnd = useWorkletCallback(function handleOnEnd(source, _ref3) {
    let {
      translationY,
      absoluteY,
      velocityY
