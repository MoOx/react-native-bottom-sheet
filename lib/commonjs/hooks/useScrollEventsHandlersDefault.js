...
const useScrollEventsHandlersDefault = (scrollableRef, scrollableContentOffsetY) => {
  ...
  const handleOnScroll = (0, _reactNativeReanimated.useWorkletCallback)((_, context) => {
    ...
    if (animatedScrollableState.value === _constants.SCROLLABLE_STATE.LOCKED) {
      const lockPosition = context.shouldLockInitialPosition ? context.initialContentOffsetY ?? 0 : 0; // @ts-ignore
  ...
  const handleOnBeginDrag = (0, _reactNativeReanimated.useWorkletCallback)((_ref, context) => {
    let {
      contentOffset: {
        y
      }
  ...
  const handleOnEndDrag = (0, _reactNativeReanimated.useWorkletCallback)((_ref2, context) => {
    let {
      contentOffset: {
        y
      }
  ...
  const handleOnMomentumEnd = (0, _reactNativeReanimated.useWorkletCallback)((_ref3, context) => {
    let {
      contentOffset: {
        y
      }
